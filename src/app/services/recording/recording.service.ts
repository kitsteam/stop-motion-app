...
export class RecordingService {
    ...
    public async convertAudioBlob(audioBlob: Blob, mimeType: string = MimeTypes.audioWebm): Promise<Blob> {
        if (!audioBlob) {
        ...
        const audioContext = await this.createAudioRecordingContext(audioBlob);
        if (!audioContext) {
            console.warn('[RecordingService] Falling back to original audio blob due to missing AudioContext support.');
            return audioBlob;
        }
    ...
    private resolveMimeType(preferred?: string, fallbackMimeTypes: string[] = this.preferredVideoMimeTypes): string | undefined {
    ...
    private async decodeFrame(blob: Blob): Promise<DrawableFrame> {
        const win = this.document.defaultView as (Window & { webkitCreateImageBitmap?: typeof createImageBitmap }) | null;
    ...
    private async createAudioRecordingContext(audioBlob?: Blob): Promise<AudioRecordingContext | null> {
        if (!audioBlob) {
            return null;
        }

        const win = this.document.defaultView as ({ AudioContext?: typeof AudioContext; webkitAudioContext?: typeof AudioContext } | null);
        const AudioContextCtor = win?.AudioContext || win?.webkitAudioContext;
        if (!AudioContextCtor) {
            console.warn('[RecordingService] AudioContext is not supported. Audio track will be omitted.');
            return null;
        }
